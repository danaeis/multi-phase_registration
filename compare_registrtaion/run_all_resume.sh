...
# start_watcher <sentinel> <log> <conditions...>
...
job_a1_a2() {
    hdr "A1 crop-only + A2 z-align  (data on disk, eval now)"
    if is_done "eval_a1_a2"; then
        echo "[$(ts)] SKIP  eval_a1_a2"
        return 0
    fi
    echo "[$(ts)] START eval A1_crop_only + A2_zalign"
...
job_a_series_aligned() {
    ...
    # ── Stage 1: rigid registration (aligned) ────────────────────────────
    local s_rigid="$LOG_DIR/.rigid_aligned_done"
    rm -f "$s_rigid"

    local w_rigid
    w_rigid=$(start_watcher "$s_rigid" "$LOG_DIR/a345_aligned_eval.log" \
        A3_pass0 A4_pass01 A5_pass012)

    run_reg "rigid_aligned" "$LOG_DIR/rigid_aligned_reg.log" \
        $PYTHON ../registration/register_fixed.py --all --skip || true

    touch "$s_rigid"
    wait "$w_rigid"
    echo "[$(ts)] rigid aligned done + A3/A4/A5 evaluated"

    # ── Stage 2: deformable registration (aligned, sobel_ncc) ────────────
    ...
    touch "$s_deform"
    wait "$w_deform"
    echo "[$(ts)] deformable aligned done + A6_bspline evaluated"
}
...
job_a_series_baseline() {
    ...
    # ── Stage 1: rigid registration (baseline / no z-align) ──────────────
    ...
    run_reg "rigid_baseline" "$LOG_DIR/rigid_baseline_reg.log" \
        $PYTHON ../registration/register_fixed.py --all --skip --baseline || true

    touch "$s_rigid_b"
    wait "$w_rigid_b"
    echo "[$(ts)] rigid baseline done + A3b/A4b/A5b evaluated"

    # ── Stage 2: deformable registration (baseline, sobel_ncc) ───────────
    ...
    run_reg "a6_sobelncc_baseline" "$LOG_DIR/a6_baseline_reg.log" \
        $PYTHON ../registration/deformable_registration_v2.py \
            --metric sobel_ncc --all --skip --baseline || true
    ...
    touch "$s_deform_b"
    wait "$w_deform_b"
    echo "[$(ts)] deformable baseline done + A6_bspline_baseline evaluated"
}
...
job_unigradicon_io() {
    ...
    local w; w=$(start_watcher "$sentinel" "$LOG_DIR/unigradicon_io_eval.log" \
        B5_unigradicon_io__aligned B5_unigradicon_io__baseline B5_unigradicon_io__raw)
